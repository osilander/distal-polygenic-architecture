...
simplify_go_dt <- function(dt) {
  ...
  for (src in go_sources) {
    ...
    sub <- dt[source == src]
    if (is.null(gd)) { keep_rows <- c(keep_rows, list(sub)); next }

    groups <- unique(sub[, .(pc, band, dist_mode)])
    for (g in seq_len(nrow(groups))) {
      grp     <- groups[g]
      grp_sub <- sub[pc == grp$pc & band == grp$band & dist_mode == grp$dist_mode]
      if (nrow(grp_sub) <= 1L) { keep_rows <- c(keep_rows, list(grp_sub)); next }
...
bands_show <- c("Top 2%")   # only band produced by compute-full-gene-enrichment.R
...
for (band_show in bands_show) {
  ...
  for (dm in unique(dt_s$dist_mode)) {
    ...
    fname <- file.path("figures",
      sprintf("figureS7_full_enrichment_dotplot_%s_%s.pdf", dm, band_slug[[band_show]]))
    ggsave(fname, fig, width = width, height = height)
    message("Wrote: ", fname)
  }
}

message("run-figureS-full-enrichment complete.")
