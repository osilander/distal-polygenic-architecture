...
```
distal-polygenic-architecture/
  data/                    → symlink to source data tree (read-only)
  ...
  README.md
```
> **`data/` symlink:** this repo assumes `data/` points to the source manuscript data tree. Set it up with:
> ```bash
...
Copy from the source results archive if rebuilding from scratch.
